...
import { Package, Calendar, MapPin, CreditCard, Clock, CheckCircle, XCircle, Truck } from "lucide-react"
...
function MisPedidos() {
  ...
  useEffect(() => {
    const obtenerPedidos = async () => {
      ...
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API}/todosroles/pedidos/ObtenerPorCliente/${usuarioId}`, {
          headers: {
  ...
  useEffect(() => {
    if (pedidos.length > 0) {
      console.log("Estados únicos encontrados:", [...new Set(pedidos.map((p) => p.estado.toLowerCase().trim()))])
      console.log("Total de pedidos:", pedidos.length)
      console.log("Filtro actual:", filtroEstado)
      console.log("Pedidos filtrados:", pedidosFiltrados.length)
    }
  }, [pedidos, filtroEstado, pedidosFiltrados.length])
    ...
    <div className="Usuario-layout">
      ...
      <main className={`content ${isCollapsed ? "collapsed" : ""}`}>
        <div className="pedido-header-section">
          ...
          <p className="pedido-subtitle">Gestiona y revisa el estado de todos tus pedidos</p>
        </div>
            ...
            {pedidosFiltrados.length === 0 ? (
              <div className="pedido-empty-state">
                ...
                <h3 className="pedido-empty-title">
                  {filtroEstado === "todos" ? "No tienes pedidos realizados" : `No tienes pedidos ${filtroEstado}s`}
                </h3>
                  ...
                  return (
                    <div key={pedido.idPedido} className="pedido-card">
                      <div className="pedido-card-header">
                        ...
                        <div className={`pedido-estado-badge ${getEstadoClass(pedido.estado)}`}>
                          {getEstadoIcon(pedido.estado)}
                          <span className="pedido-estado-text">{pedido.estado}</span>
                        </div>
                      </div>
                      <div className="pedido-card-body">
                        <div className="pedido-info-grid">
                          <div className="pedido-info-item">
                            ...
                            <div className="pedido-info-content">
                              ...
                              <span className="pedido-info-value">{pedido.detalles.length}</span>
                            </div>
                          </div>
                          <div className="pedido-info-item">
                            ...
                            <div className="pedido-info-content">
                              ...
                              <span className="pedido-info-value pedido-total-amount">S/. {total.toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
                        <div className="pedido-direccion-container">
                          ...
                          <div className="pedido-direccion-content">
                            <span className="pedido-direccion-label">Dirección de entrega</span>
                            <span className="pedido-direccion-text">{pedido.direccion}</span>
                          </div>
                        </div>
                      </div>
                      <div className="pedido-card-footer">
                        <button className="pedido-details-button" onClick={() => abrirDetalles(pedido)}>
                          Ver detalles
                        </button>
                        {pedido.estado.toLowerCase() === "pendiente" && (
                          <button className="pedido-cancel-button">Cancelar pedido</button>
                        )}
                      </div>
                    </div>
                  )
        ...
        {pedidoSeleccionado && <PedidoDetalleModal pedido={pedidoSeleccionado} onClose={cerrarDetalles} />}
      </main>
    </div>
  )
}
